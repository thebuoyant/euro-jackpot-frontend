...
import WinningNumbers123Toolbar from "./WinningNumbers123Toolbar";

type WinningNumbersItem = { key: number; value: number };

export default function WinningNumbers123Page() {
  const {
    setIsLoadingWinningNumbers123,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setIsLoadingWinningNumbers123(true);

        const res = await fetch(
          `${API_ROUTE_CONST.winningNumbersTop123}?sortedValues=${showSortedValues123}`,
          { signal: ac.signal }
        );
        ...
        setWinningNumbersCounts123(Array.isArray(data) ? data : []);
      } catch (err: any) {
        ...
        setIsLoadingWinningNumbers123(false);
      }
    })();
  ...
  return (
    <div className="winning-numbers-123-page">
      <div className="winning-numbers-page-header page-header">
        <Typography variant="h6">
          {APP_TYPO_CONST.pages.winningNumbers123.headerTitle}
        </Typography>
      </div>

      <div className="winning-numbers-123-toolbar">
        <WinningNumbers123Toolbar />
      </div>
      ...
      <div className="winning-numbers-page-content page-content">
        <div className="wining-numbers-wrapper">
          <ResponsiveContainer width="100%" height={600}>
            <BarChart
              data={winningNumbersCounts123} // [{ key, value }]
            ...
            >
              ...
              <Bar dataKey="value" name="Einsatz" isAnimationActive={false}>
                ...
                />
                {winningNumbersCounts123.map((_: any, idx: number) => (
                  <Cell
                    key={`cell-${idx}`}
